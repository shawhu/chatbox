...
export default function SettingWindow(props: Props) {
  ...
  const [settingsEdit, setSettingsEdit] = React.useState<Settings>(
    props.settings
  );
  ...
  const handleRepliesTokensInputChange = (
  ...
  const handleMaxContextInputChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const value = event.target.value;
    if (value === "inf") {
      ...
      <DialogContent>
        ...
        <FormGroup>
          <FormControlLabel
            ...
            onChange={(e, checked) =>
              setSettingsEdit({ ...settingsEdit, showWordCount: checked })
            }
          />
        </FormGroup>
        <FormGroup>
          <FormControlLabel
            ...
            onChange={(e, checked) =>
              setSettingsEdit({ ...settingsEdit, showTokenCount: checked })
            }
          />
        </FormGroup>
        <Accordion>
          ...
          <AccordionDetails>
            ...
            {!settingsEdit.apiHost.match(
              ...
              <Alert severity="warning">
                {t("your api key and all messages will be sent to")}{" "}
                <b>{settingsEdit.apiHost}</b>.
                {t(
                ...
                <Button
                  onClick={() =>
                    setSettingsEdit({
                      ...settingsEdit,
                      apiHost: getDefaultSettings().apiHost,
                    })
                  }
                >
                  {t("reset")}
            ...
            {settingsEdit.apiHost.startsWith("http://") && (
              <Alert severity="warning">
                {t("all data transfers are being conducted through the")}{" "}
                <b>{t("http")}</b>{" "}
                {t(
                  "protocol, which may lead to the risk of api key and data leakage."
                )}
                {t(
                  "unless you are completely certain and understand the potential risks involved, please consider using the"
                )}{" "}
                <b>{t("https")}</b> {t("protocol instead.")}
              </Alert>
            )}
            {!settingsEdit.apiHost.startsWith("http") && (
              <Alert severity="error">
                {t("proxy must use")} <b> {t("http")} </b> {t("or")}{" "}
                <b> {t("https")} </b> {t("proxy api host alert end")}
              </Alert>
            )}
          </AccordionDetails>
        </Accordion>
        <Accordion>
          ...
          <AccordionDetails>
            <Alert severity="warning">
              {t(
                "these settings are aimed at professional developers. if you do not understand the meaning of these settings, please do not modify them, as it may result in request errors."
              )}
              {t(
                "before making any modifications, please verify that your account has access to the selected models (some models require additional joining of the waiting list, regardless of your account type, otherwise, it will result in 404 errors)."
              )}
              {t(
                "please make sure that the number of tokens does not exceed the limit for the selected model, otherwise, an error message will occur once the context exceeds the limit."
              )}
            ...
            >
              <Box sx={{ width: "92%" }}>
                <Slider
                  value={
                    settingsEdit.maxTokens === "inf"
                      ? 8192
                      : Number(settingsEdit.maxTokens)
                  }
                  defaultValue={
                    settingsEdit.maxTokens === "inf"
                      ? 8192
