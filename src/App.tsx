...
function Main() {
  ...
  useEffect(() => {
    if (
      ...
      store.currentSession.messages.findIndex(
        ...
        >
          ...
          >
            ...
            <MenuList
              ...
              className="scroll"
              subheader={
                <ListSubheader component="div">{t("chat")}</ListSubheader>
              }
              component="div"
              ref={sessionListRef}
              // dense
            ...
            >
              {store.currentSession.messages.map((msg, ix) => (
                <Block
                  ...
                  setMsg={(updated) => {
                    store.currentSession.messages =
                      store.currentSession.messages.map((m) => {
                        if (m.id === updated.id) {
                          return updated;
                        }
                        return m;
                  ...
                  refreshMsg={() => {
                    if (msg.role === "assistant") {
                      const promptMsgs = store.currentSession.messages.slice(
                        0,
                        ix
                      );
                      generate(store.currentSession, promptMsgs, msg);
                    } else {
                      const promptsMsgs = store.currentSession.messages.slice(
                        0,
                        ix + 1
                      );
    ...
    >
      <Stack direction="column" spacing={1}>
        ...
        <Typography variant="caption" style={{ opacity: 0.3 }}>
          {t(
            "[Enter] new line, [Ctrl+Enter] send, [Shift+Enter] send without generating"
          )}
        </Typography>
      </Stack>
    </form>
